...
function brandExtractionPrompt(input: {
  ...
  return [
    ...
    '',
    '1. MEASURE — drive the site with agent-browser. Snapshot it, then harvest the real design language: frequency-ranked color literals including oklch() and `<meta name="theme-color">` (background / surface / foreground / muted / border / accent / accent-secondary), the @font-face + font-family declarations (follow Google Fonts preload/stylesheet links), and representative headings + copy for voice.',
    '   - LOGO (extract MULTIPLE candidates): save every logo you can find as a file under `logos/` — the inline header/nav SVG wordmark (skip 24px menu glyphs; write the literal `<svg>…</svg>` markup verbatim to `logos/header.svg`, do NOT just reference it), header `<img>` including srcset/data-src, JSON-LD Organization.logo, web-app manifest icons, apple-touch-icon, mask-icon, favicon, and og:image. Set `logo.primary` to the best vector/transparent lockup and list the rest in `logo.alternates` (the kit page shows them as switchable thumbnails). NEVER leave `logo.primary` empty when the site has any mark — fetch the asset URLs directly and save real files. (The daemon also auto-fetches a favicon/og:image fallback so the page is never logo-less, but that is a safety net, not a substitute for the real wordmark.)',
