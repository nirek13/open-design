...
function renderMetadataBlock(
  ...
  if (factsOnly && metadata.includeLandingPage) {
    lines.push(
      '- **includeLandingPage**: true — ship `landing.html` as a separate responsive marketing surface. Treat it as a studio-grade campaign site: named visual direction, cinematic hero, distinctive type, working CTA; not a generic three-card SaaS template.',
    );
  }
  if (!factsOnly && metadata.includeLandingPage) {
    lines.push(
      '- **includeLandingPage**: true — create `landing.html` as a separate responsive marketing companion surface in addition to the selected product/app screens. Do not implement the landing page only as a section inside `index.html`, even for responsive-web-only projects. If there is a working product/app screen, create it as a separate file such as `app.html`, `dashboard.html`, or a domain-specific screen name. `index.html` should be a lightweight launcher/overview when multiple files exist. Treat `landing.html` as a studio-grade campaign site: commit to one named visual direction, cinematic full-bleed hero with a real product moment, distinctive typography (not Inter/Roboto), one accent color, specific copy (no "reimagine/unlock/elevate"), and a working primary CTA. Forbidden: three equal feature cards with line icons, indigo gradients, beige AI canvas, blob illustrations. Include proof, product-in-use, and an appropriate CTA such as waitlist, download, or contact sales. It must feel stunning at both 375px and 1440px.',
    );
  }
