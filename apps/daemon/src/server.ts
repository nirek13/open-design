...
export function createAgentRuntimeToolPrompt(
  ...
  const pagesBlock = allow(INTERNAL_PAGES)
    ? [
        ...
        '- Write content with `tools pages upsert --input page.json` (create or replace) or `tools pages append --page <id> --input blocks.json`. Block types: paragraph, heading_1/2/3, bulleted_list_item, numbered_list_item, to_do, toggle, callout, quote, code, divider, bookmark, embed, image, video, audio, file, pdf, equation, table_of_contents, breadcrumb, column_list, column, table, database, artifact, page, record, board, checklist, assigner, poll, timeline, decision, goals, spreadsheet, budget, calendar, habit, countdown, schedule. Prefer native page tools over generated HTML artifacts: `board` (kanban), `checklist`, `assigner`, `poll`, `timeline`, `decision`, `goals`, `spreadsheet`, `budget`, `calendar`, `habit`, `countdown`, `schedule`. When the user wants a tool, tracker, dashboard, kanban, checklist, assignment board, poll, timeline, decision log, goals, spreadsheet, budget, calendar, habit tracker, countdown, or weekly schedule, append that block with a filled `content` object (`kind` plus columns/items/tasks/options/cells) — do not generate HTML/JS and embed it. Headings can be collapsible with `props.toggle`. Page appearance is `style: { font, smallText, fullWidth, locked }`.',
  ...
  const mailBlock = allow(INTERNAL_MAIL)
    ? [
        ...
        '### Mail (`tools mail`)',
        '',
        '- Read, send, and sort the organization mailbox (same Gmail connection as the Mail UI). Prefer `tools mail` when the user asks to check email, send a note, reply, or have you triage the inbox.',
        ...
        '- Sort and mark: `tools mail triage --apply` (stars asks, archives bulk, marks FYI read). Preview without `--apply`. Draft: `tools mail draft --thread <thread-id>`. Summarize: `tools mail summarize --thread <thread-id>`. Star/archive: `tools mail modify --message <id> --add STARRED --remove INBOX`.',
