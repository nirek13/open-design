...
describe('UpdaterPopup', () => {
  ...
  it('shows only the ready indicator until the user opens the install prompt', async () => {
    ...
    expect(screen.getByText('Substrate 1.2.3-beta.4 is ready. Substrate will close and open the installer.')).toBeTruthy();
  ...
  it('uses reinstall copy and the learn-more link for forced installer reinstalls', async () => {
    ...
    expect(
      screen.getByText('Substrate 1.2.3-beta.4 requires a full reinstall. Substrate will close and open the installer.'),
    ).toBeTruthy();
  ...
  it('uses localized ready prompt copy from the app i18n provider', async () => {
    ...
    expect(screen.getByText('Substrate 1.2.3-beta.4 已就绪。Substrate 会关闭并打开安装器。')).toBeTruthy();
  });
  ...
  it('uses install-and-restart copy for payload updates', async () => {
    ...
    expect(screen.getByText('Substrate 1.2.3-beta.4 已就绪。Substrate 会关闭并自动重启。')).toBeTruthy();
  });
    ...
    await act(async () => {
      status = downloadedStatus({
        installResult: {
          ...
          path: '/tmp/open-design-updater/Substrate Beta.dmg',
        },
      });
  ...
  it('recovers the handoff prompt if the app has not closed after the watchdog', async () => {
    ...
    try {
      ...
      expect(screen.getByTestId('updater-install-button').textContent).toBe('Quit Substrate');
