...
const defaultMessages: MessageCenterMessage[] = [
  { id: 'release', audienceType: 'global', typeName: 'Product update', title: 'Substrate 0.14 is available', body: 'The new release is ready.', ctaLabel: 'View update', ctaUrl: 'https://open-design.ai/update', publishedAt: '2026-07-16T12:00:00.000Z', readAt: null },
...
describe('MessageCenter', () => {
  ...
  it('renders API messages for anonymous clients without a local window', async () => {
    ...
    expect(within(dialog).getByText('Substrate 0.14 is available')).toBeTruthy();
  ...
  it('keeps anonymous read state locally and restores it', async () => {
    renderMessageCenter();
    await openCenter();
    fireEvent.click(screen.getByRole('button', { name: /Substrate 0\.14 is available/ }));
  ...
  it('filters messages and marks all read', async () => {
    ...
    expect(screen.getByText('Substrate 0.14 is available')).toBeTruthy();
  ...
  it('keeps visible account messages when locale changes and the follow-up sync fails', async () => {
    ...
    await openCenter();
    await waitFor(() => expect(screen.getByText('Substrate 0.14 is available')).toBeTruthy());
