...
describe('MemorySection', () => {
  ...
  it('renders the memory tree and opens an entry editor from a tree node', async () => {
    ...
    const entry = {
      ...
      description: 'Substrate should evolve from accepted work',
      type: 'project',
          ...
          suggestions: [
            {
              ...
              type: 'project',
              body: 'Substrate connector memory should focus on design preferences, UI decisions, and visual references from Notion.',
              source: {
                kind: 'connector',
                connectorId: 'notion',
  ...
  it('renders connector model authentication failures without raw provider JSON', async () => {
    ...
    globalThis.fetch = vi.fn(async (input: RequestInfo | URL) => {
          ...
          extractions: [
            {
              ...
              userMessagePreview: 'Suggest durable Substrate memories from connected apps.',
	  ...
	  expect(
	    screen.queryByText('Suggest durable Substrate memories from connected apps.'),
	  ).toBeNull();
	  expect(
	    within(document.querySelector('.memory-unified-list') as HTMLElement)
	      .queryByText('Suggest durable Substrate memories from connected apps.'),
	  ).toBeNull();
	  expect(screen.getByText('OpenAI authentication expired')).toBeTruthy();
    expect(
      screen.getByText('Connected apps were read, but Substrate could not turn that context into memory.'),
    ).toBeTruthy();
    expect(
