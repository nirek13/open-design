    ...
    return (
      <div className="ds-setup-shell ds-setup-shell--center">
        <div className="ds-setup-center-card">
          <h1>Substrate will extract your design system.</h1>
  ...
  const sendProjectChatMessage = useCallback(
    async (
      ...
      if (config.mode !== 'daemon' || !config.agentId) {
        setChatError('Pick a local agent first, then ask Substrate to update this design system.');
        return;
      }
              ...
              <p>
                {generationActive
                  ? activeJob?.kind === 'token-contract-rebuild'
                    ? 'Substrate is preparing a token contract rebuild for review. The active contract stays unchanged until you accept it.'
                    : activeJob?.kind === 'revision'
                      ? 'Substrate is applying your feedback. You can keep reviewing while the updated draft is prepared.'
                      : 'Substrate is still working, but you can start giving feedback on the work so far.'
                  : 'Substrate is ready for review. Give feedback on the work so far, then publish when it is useful for future projects.'}
              </p>
              <label>
                <input
                  type="checkbox"
    ...
    <section className={`ds-workspace-activity-card is-${status}`}>
      <div className="ds-workspace-activity-head">
        ...
        <span>
          <strong>
            {status === 'running'
              ? 'Substrate is updating this system'
              : status === 'failed'
          ...
          <strong>
            {active
              ? job.kind === 'token-contract-rebuild'
                ? 'Substrate is rebuilding tokens'
                : job.kind === 'revision'
                  ? 'Substrate is revising'
                  : 'Substrate is still working'
              : job.status === 'failed'
...
function GitHubRepositoryAccessPanel({
  ...
  const methods: GitHubAccessMethod[] = [
    ...
    },
    {
      ...
      badge: 'Coming soon',
      tone: 'muted',
      description: 'Native GitHub sign-in managed by Substrate; this build does not use an OD-managed GitHub token yet.',
    },
    {
      id: 'composio',
      icon: composioIcon,
...
function buildFallbackDesignMdFromState(state: SetupState): string {
  ...
  const overview =
    ...
    || state.notes.trim()
    || sourceNotes
    || 'Design system generated from source material supplied in Substrate.';
  return [
    '---',
...
function buildCreationAgentPrompt(
  ...
  const title = inferDesignSystemTitle(state);
  return [
    'Create this project as a complete Substrate design system workspace.',
    '',
    ...
    state.codeFolders.length
      ? `Read the linked local code folders that Substrate attached to this project: ${state.codeFolders.join(', ')}. Treat them as source context only unless the user asks you to edit them.\n\n${localFolderRunbook}`
      : '',
...
function buildSourceContextManifest(
  ...
  sections.push(
    ...
    '- README.md and SKILL.md should make the extracted system reusable as a real Substrate design-system package.',
