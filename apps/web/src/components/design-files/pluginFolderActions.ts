...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the Substrate community catalog at `nexu-io/open-design`.',
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'Run this deterministic Substrate CLI workflow from the current project workspace:',
    '',
...
// Substrate issue URL and belongs to the "Substrate PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits an Substrate issue URL and belongs to the "Substrate PR" button.',
    '',
    'Run this deterministic Substrate CLI workflow from the current project workspace:',
    '',
