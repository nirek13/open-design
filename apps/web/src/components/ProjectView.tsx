...
type ProjectChatSendMeta = ChatSendMeta & {
  ...
  queueDrain?: boolean;
  /** The Substrate Cloud balance gate already ran for this exact send at
...
function historyWithWorkspaceContext(
  ...
  const block = [
    ...
    'Substrate selected or inferred these workspace contexts for this turn. Treat absolute paths as reference context unless the user explicitly asks to edit them.',
...
/** Set by the home create flow when its submit already ran the Substrate
...
export function ProjectView({
  ...
  const handleSend = useCallback(
    async (
      ...
      // Substrate Cloud pre-run balance gate: a definitively insufficient
  ...
  // "Share to Substrate" — kicks off the bundled `od-share-to-community`
  ...
  const [shareToOpenDesignBusyMessageId, setShareToSubstrateBusyMessageId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    setShareToSubstrateBusyMessageId(null);
  }, [currentConversationBusy]);
  const handleShareToSubstrate = useCallback((assistantMessageId: string) => {
    ...
    setShareToSubstrateBusyMessageId(assistantMessageId);
        ...
        <div className="split-chat-slot" hidden={workspaceFocused}>
          {commentInspectorActive ? (
            ...
            <ChatPane
              ...
              onShareToSubstrate={handleShareToSubstrate}
...
function pluginWorkflowTitle(action: PluginFolderAgentAction): string {
  return action === 'publish' ? 'Publish repo' : 'Substrate PR';
}
...
function pluginWorkflowPlannedSteps(action: PluginFolderAgentAction): string[] {
  ...
  return [
    'Ensure the Substrate fork exists',
...
const DESIGN_RESULT_DELIVERY_FAILED_DETAIL =
  'The design result was generated, but Substrate could not save it to the project.';
