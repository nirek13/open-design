// Content fixtures for the "Use Substrate everywhere" guide modal.
//
...
export const GUIDE_SECTIONS: GuideSection[] = [
  {
    id: 'overview',
    tabLabel: 'Overview',
    heading: 'Substrate works wherever your agent works',
    intro:
      'Substrate is more than a window — it is a local privileged daemon ' +
    ...
    bullets: [
      ...
      'MCP server — wires Substrate as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
      'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE endpoints; the same surface the web UI uses.',
      'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without Substrate at all.',
    ...
    snippets: [
      ...
      },
      {
        ...
        body:
          ...
          '# Then start Substrate in the shell you normally use for dev:\n' +
          'pnpm tools-dev',
      },
    ],
    footer:
  ...
  },
  {
    id: 'cli',
    tabLabel: 'CLI · od',
    heading: 'Drive Substrate from any shell',
    intro:
  ...
  },
  {
    id: 'mcp',
    tabLabel: 'MCP server',
    heading: 'Expose Substrate as an MCP server to any coding agent',
    intro:
      'Substrate ships with a Model Context Protocol server (`od mcp`) ' +
      'that lets any MCP-capable client — Cursor, Claude Code, Antigravity, ' +
      'VS Code Copilot Chat, openclaw, hermes — discover Substrate tools ' +
    ...
    footer:
      'In the Substrate app, open Settings → Integrations to copy a ' +
  ...
  },
  {
    ...
    heading: 'Drop-in Skills for any agent — even without Substrate running',
    intro:
      'A Skill is a directory with a Claude-compatible `SKILL.md` ' +
      '(YAML front-matter + body). Substrate extends the format with the ' +
      '`od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so ' +
      'the same artifact can be used both inside Substrate and by a vanilla ' +
    ...
    snippets: [
      {
        label: 'Minimal SKILL.md (Claude-compatible front matter + Substrate extras)',
        language: 'yaml',
        body:
          '---\n' +
      ...
      },
      {
        ...
        body:
          ...
          '# Shell 1: start Substrate after ingesting.\n' +
          'pnpm tools-dev\n' +
