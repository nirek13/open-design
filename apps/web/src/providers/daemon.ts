...
function truncateForTranscript(content: string): string {
  ...
  return `${content.slice(0, MAX_TRANSCRIPT_MESSAGE_CHARS)}\n\n[Substrate truncated ${omitted} chars from this prior message before sending it to the agent. Full content remains in persisted history.]`;
}
...
const AMR_OPENCODE_INCOMPLETE_MESSAGE =
  'Substrate started, but the run did not complete. Please retry or check the run details for the session stream error.';
...
function formatOpenCodeSessionError(value: unknown): string | null {
  ...
  if (statusCode === 404) {
    return 'The model service returned 404 Not Found for the configured runtime endpoint. Check the Substrate link URL or model route.';
  }
  if (statusCode === 401 || statusCode === 403) {
    return 'Substrate authentication failed. Please sign in again or refresh the runtime key.';
  }
  if (statusCode === 429) {
