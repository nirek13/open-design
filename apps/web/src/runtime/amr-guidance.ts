...
// attribute the visit to Substrate (per-product revenue/traffic attribution).
...
const AGENT_AGNOSTIC_DETAIL_FAILURE_UI: Record<string, RunFailureUi> = {
  ...
  // The fix is updating Substrate to a build that bundles a compatible
  ...
  if (agentId === 'amr') {
    if (code === 'AMR_AUTH_REQUIRED') {
      return {
        ...
        // "Substrate 智能体尚未登录，前往登录即可正常使用" — single CTA, no
