    ...
    <script>
      (function(){
        ...
        try {
          ...
          (0, eval)(compiled);
          var Component = window.__SubstrateComponent ||
...
window.__SubstrateComponent = window.__SubstrateComponent || (${componentFallbackExpression(transformed.defaultName)});`;
}
...
function transformExports(source: string): { code: string; defaultName: string | null } {
  ...
  code = code.replace(
    ...
    (_match, name: string | undefined) => {
      defaultName = name || 'SubstrateComponent';
  ...
  code = code.replace(/export\s+default\s+/g, () => {
    defaultName = 'SubstrateComponent';
    return 'const SubstrateComponent = ';
  });
  code = code.replace(
