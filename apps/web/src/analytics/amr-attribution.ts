...
// Builds the AMR handoff URL with Substrate attribution params. When
// `deviceId` is provided it is added as `od_device_id`, so AMR can link the
// landing/registration directly back to this Substrate install instead of
// only through the one-shot entry id. The caller passes it ONLY when the user
// has consented to metrics: AMR is Substrate's official model service, so
...
async function mirrorAmrEntryToAmrAnalytics(
  ...
  try {
    ...
    // AMR analytics mirroring must never block the primary Substrate action.
  }
}
