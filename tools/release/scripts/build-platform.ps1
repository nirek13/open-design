...
function Validate-WinLauncherPayloadArchive([string]$PayloadPath, [string]$ExpectedVersion, [string]$Label) {
  ...
  try {
    ...
    Test-JsonString $manifest.entry.executable "entry.executable" "payload/Substrate.exe"

    $entryPath = Join-Path $extractRoot (Convert-ArchiveRelativePath "payload/Substrate.exe")
