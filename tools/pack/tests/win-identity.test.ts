...
describe("resolveWinInstallIdentity", () => {
  it("keeps the default namespace on the canonical Windows display name", () => {
    expect(resolveWinInstallIdentity({ namespace: "default" })).toMatchObject({
      ...
      shortcutName: "Substrate.lnk",
      uninstallerName: "Uninstall Substrate.exe",
    });
  });
  ...
  it("uses the canonical Windows display name for stable release namespaces", () => {
    expect(resolveWinInstallIdentity({ namespace: "release-stable-win" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Substrate.exe",
      displayName: "Substrate",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Substrate-release-stable-win",
      shortcutName: "Substrate.lnk",
      uninstallerName: "Uninstall Substrate.exe",
    });
  });
  ...
  it("uses first-class beta display identity for beta release namespaces", () => {
    expect(resolveWinInstallIdentity({ namespace: "release-beta-win" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Substrate Beta.exe",
      displayName: "Substrate Beta",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Substrate-release-beta-win",
      shortcutName: "Substrate Beta.lnk",
      uninstallerName: "Uninstall Substrate Beta.exe",
    });
  });
  ...
  it("keeps non-release beta-like namespaces isolated from the real beta channel identity", () => {
    expect(resolveWinInstallIdentity({ namespace: "beta-local-flow" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Substrate beta-local-flow.exe",
      displayName: "Substrate beta-local-flow",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Substrate-beta-local-flow",
      shortcutName: "Substrate beta-local-flow.lnk",
      uninstallerName: "Uninstall Substrate beta-local-flow.exe",
    });
  });
  ...
  it("uses first-class preview display identity for preview release namespaces", () => {
    expect(resolveWinInstallIdentity({ namespace: "release-preview-win" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Substrate Preview.exe",
      displayName: "Substrate Preview",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Substrate-release-preview-win",
      shortcutName: "Substrate Preview.lnk",
      uninstallerName: "Uninstall Substrate Preview.exe",
    });
  });
  ...
  it("uses first-class prerelease display identity for prerelease release versions and namespaces", () => {
    expect(resolveWinInstallIdentity({
      ...
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Substrate Prerelease.exe",
      displayName: "Substrate Prerelease",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Substrate-release-stable-win",
      shortcutName: "Substrate Prerelease.lnk",
      uninstallerName: "Uninstall Substrate Prerelease.exe",
    });
