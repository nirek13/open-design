...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Substrate-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Substrate-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Substrate Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Substrate Beta", "Substrate.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Substrate-release-beta-win-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Substrate-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Substrate-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Substrate Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "Substrate.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "Substrate.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "Substrate.exe"),
      source: "namespace",
...
describe("tools-pack launcher payload archives", () => {
  it("builds channel and namespace scoped payload manifests for both desktop platforms", () => {
    ...
    expect(buildMacLauncherPayloadManifest({
      ...
      appBundleName: "Substrate Beta.app",
      channel: "beta",
      entry: {
        cwd: "payload/Substrate Beta.app",
        executable: "payload/Substrate Beta.app/Contents/MacOS/Substrate Beta",
      },
      namespace: "release-beta",
      payloadRoot: "payload",
      platform: "darwin",
    ...
    expect(buildWinLauncherPayloadManifest({
      ...
      entry: {
        cwd: "payload",
        executable: "payload/Substrate.exe",
      },
      namespace: "release-beta-win",
      payloadRoot: "payload",
      platform: "win32",
  ...
  it.skipIf(process.platform !== "darwin")("creates a mac payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("Substrate Beta.app");
      expect(manifest.entry.executable).toBe("payload/Substrate Beta.app/Contents/MacOS/Substrate Beta");
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/Substrate.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "Substrate.exe"));
