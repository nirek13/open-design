...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Substrate-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Substrate-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Substrate"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Substrate", "Substrate.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Substrate-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Substrate-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Substrate-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Substrate", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Substrate.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "Substrate.exe"))).resolves.toMatchObject({
